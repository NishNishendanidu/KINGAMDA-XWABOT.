...
badboy.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram", dontAddCommandList: true }, async (message, match) => { 
    const { status, type, data } = await instagram(match[1], 'badboy')
    if (!status) return await message.sendMessage('not found')
    await message.client.sendMessage(message.jid, LOAD_ING, MessageType.text, { quoted: message.data });
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "𝙼𝚊𝚍𝚎 𝚋𝚢 𝙱𝚊𝚍𝚋𝚘𝚢🤪", quoted: message.data })
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "𝙼𝚊𝚍𝚎 𝚋𝚢 𝙱𝚊𝚍𝚋𝚘𝚢🤪", quoted: message.data })
});
