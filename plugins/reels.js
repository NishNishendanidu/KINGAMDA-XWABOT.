...
Asena.addCommand({pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram" , dontAddCommandList: true }, async (message, match) => {
   ...
   const { status, type, data } = await instagram(match[1], 'julie')
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "*𝙼𝚊𝚍𝚎 𝚋𝚢 𝙱𝙰𝙳𝙱𝙾𝚈😈*", quoted: message.data })
    
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "*𝙼𝚊𝚍𝚎 𝚋𝚢 𝙱𝙰𝙳𝙱𝙾𝚈😈*", quoted: message.data })
